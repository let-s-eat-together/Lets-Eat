...
class KeychainManager {
    // MARK: [키 체인 클래스 사용 방법]
    /*
     1. 디바이스 정보 저장 : KeychainManager().createDeviceID() => 리턴 값 true / false
     2. 디바이스 정보 조회 : KeychainManager().getDeviceID() => 저장된 디바이스 고유값 문자열
     3. 디바이스 정보 삭제 : KeychainManager().deleteDeviceID() => 리턴 값 true / false
     */
    ...
    func createDeviceID() -> Bool {
        guard let service = self.service
            ...
            print("")
            print("[KeychainManager >> createDeviceID() : Service Check : false]")
            print("")
            return false
        }
        ...
        print("")
        print("[KeychainManager >> createDeviceID() : account : \(account)]")
        print("[KeychainManager >> createDeviceID() : service : \(service)]")
        print("[KeychainManager >> createDeviceID() : deviceID : \(deviceID)]")
        print("")
        ...
        if status == errSecSuccess {
            print("")
            print("[KeychainManager >> createDeviceID() : Success Status : \(status)]")
            print("")
            return true
        }
        else {
            print("")
            print("[KeychainManager >> createDeviceID() : Fail Status : \(status)]")
            print("")
            return false
        }
    }
    ...
    func getDeviceID() -> String {
        guard let service = self.service
            ...
            print("")
            print("[KeychainManager >> getDeviceID() : Service Check : false]")
            print("")
            return ""
        }
        
        print("")
        print("[KeychainManager >> getDeviceID() : account : \(account)]")
        print("[KeychainManager >> getDeviceID() : service : \(service)]")
        print("")
        ...
        if status == errSecSuccess {
            guard let existingItem = dataTypeRef as? [String: Any]
                ...
                print("")
                print("[KeychainManager >> getDeviceID() : Type Check : false]")
                print("")
                return ""
            }
            ...
            print("")
            print("[KeychainManager >> getDeviceID() : Success Status : \(status)]")
            print("")
            return uuidData
        }
        else if status == errSecItemNotFound || status == -25300 {
            print("")
            print("[KeychainManager >> getDeviceID() : Fail Status : 저장된 데이터가 없습니다]")
            print("uuid 생성")
            if !createDeviceID() {
                print("생성 오류")
            }
            ...
            // 처리결과가 실패라면 nil을 반환
            print("")
            print("[KeychainManager >> getDeviceID() : Fail Status : \(status)]")
            print("")
            return ""
        }
    }
    ...
    func deleteDeviceID() -> Bool {
        guard let service = self.service
            ...
            print("")
            print("[KeychainManager >> deleteDeviceID() : Service Check : false]")
            print("")
            return false
        }
        
        print("")
        print("[KeychainManager >> deleteDeviceID() : account : \(account)]")
        print("[KeychainManager >> deleteDeviceID() : service : \(service)]")
        print("")
        ...
        ] // 앱 내에서 데이터를 식별하기 위한 키에 해당하는 값 (사용자 계정)
        
        // 현재 저장되어 있는 값 삭제
        let status: OSStatus = SecItemDelete(query as CFDictionary)
        if status == errSecSuccess {
            print("")
            print("[KeychainManager >> deleteDeviceID() : Success Status : \(status)]")
            print("")
            return true
        }
        else {
            print("")
            print("[KeychainManager >> deleteDeviceID() : Fail Status : \(status)]")
            print("")
            return false
        }
    }
